...
export function AnimatedThemeToggler({
  ...
  const toggleTheme = useCallback(() => {
    ...
    const applyTheme = () => {
      ...
      // Use explicit add/remove to ensure class matches the state exactly
      if (newTheme) {
        document.documentElement.classList.add("dark")
      } else {
        document.documentElement.classList.remove("dark")
  ...
  return (
    <button
      ...
      ref={buttonRef}
      onClick={toggleTheme}
      className={cn("p-2 rounded-full cursor-pointer hover:bg-[rgba(30,50,90,0.05)] dark:hover:bg-white/10 transition-colors flex items-center justify-center border border-[rgba(30,50,90,0.1)] dark:border-white/10 shadow-sm", className)}
      {...props}
    >
      {isDark ? (
        <Sun className="w-4 h-4 text-yellow-400" />
      ) : (
        <Moon className="w-4 h-4 text-[rgba(30,50,90,0.85)]" />
      )}
